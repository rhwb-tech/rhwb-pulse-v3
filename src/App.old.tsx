...
import type { Coach, Runner, QuantitativeScoreRow } from './types/database';
...
function App() {
  ...
  // Track if we should auto-fetch data after filter changes
  ...
  useEffect(() => {
    if (email && userRole && initialLoad) {
      ...
      shouldAutoFetch.current = true; // Enable auto-fetch after initial load
    }
  ...
  // Auto-fetch data when filters change (after initial load)
  useEffect(() => {
    if (shouldAutoFetch.current && !initialLoad) {
      fetchWidgetData();
    }
    // eslint-disable-next-line
  }, [season, selectedRunner, selectedCoach, hybridToggle]);
  ...
  const handleCoachChange = (coach: string) => {
    ...
    // Auto-fetch will be triggered by useEffect when selectedCoach changes
  ...
  const handleRunnerChange = (runner: string) => {
    ...
    // Auto-fetch will be triggered by useEffect when selectedRunner changes
  };
  ...
  const handleHybridToggleChangeFromChip = async (newToggle: 'myScore' | 'myCohorts') => {
    ...
    // Auto-fetch will be triggered by useEffect when hybridToggle changes
...
WHERE season = 'Season {season}' AND email_id = '{selectedRunner || email}'
                ...
                <Box sx={{ ml: 2, mb: 2, p: 1, bgcolor: '#fff', borderRadius: 1, border: '1px solid #ccc' }}>
                  <Typography variant="body2" component="pre" sx={{ margin: 0, whiteSpace: 'pre-wrap' }}>
{`SELECT meso, max(case when category='Personal' then quant end) as quant_personal,
